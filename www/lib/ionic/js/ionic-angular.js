...
function($cacheFactory, $parse, $rootScope) {
  ...
  CollectionRepeatDataSource.prototype = {
    ...
    detachItem: function(item) {
      ...
      //If it's an outside item, only hide it. These players aren't part of collection
      //repeat's list, only sit outside
      if (item.isOutside) {
        hideWithTransform(item.element);
      // If we are at the limit of backup players, just get rid of the this element
      } else if (this.backupItemsArray.length >= this.BACKUP_ITEMS_LENGTH) {
        this.destroyItem(item);
      // Otherwise, add it to our backup players
      } else {
...
function($rootScope, $timeout) {
  ...
  CollectionRepeatManager.prototype = {
    ...
    /*
     * Pre-calculate the position of all players in the data list.
     ...
     * Also save the position in the scroller for next and previous players (if they exist)
     */
     ...
     * the data source to render the correct players into the DOM.
     */
    render: function(shouldRedrawAll) {
      var self = this;
      var i;
      var isOutOfBounds = ( this.currentIndex >= this.dataSource.getLength() );
      // We want to remove all the players and redraw everything if we're out of bounds
      ...
      // If we aren't on the first item, add one row of players before so that when the user is
      // scrolling up he sees the previous item
      var renderStartIndex = Math.max(startIndex - 1, 0);
      // Keep adding players to the 'extra row above' until we get to a new row.
      // This is for the case where there are multiple players on one row above
      // the current item; we want to keep adding players above until
      ...
      // Keep rendering players, adding them until we are past the end of the visible scroll area
      i = renderStartIndex;
      ...
      // Render two extra players at the end as a buffer
      ...
      // Remove any players that were rendered and aren't visible anymore
...
.service('$ionicScrollDelegate', delegateService([
   ...
   *     <ion-list>
   *       {% raw %}<ion-item ng-repeat="i in players">{{i}}</ion-item>{% endraw %}
   *     </ion-list>
   *   </div>
   * </ion-scroll>
   * ```
 ...
 *   <ion-list>
 *     <ion-item ng-repeat="i in players">
 *       {% raw %}Hello, {{i}}!{% endraw %}
 ...
 * thousands of players in them, and experience little to no performance penalty.
 *
 * Demo:
 *
 * The directive renders onto the screen only the players that should be currently visible.
 * So if you have 1,000 players in your list but only ten fit on your screen,
 ...
 * 2. You must explicitly tell the directive what size your players will be in the DOM, using directive attributes.
 ...
 * @usage
 *
 * #### Basic Usage (single rows of players)
 *
 ...
 *       collection-repeat="item in players"
 ...
 *     $scope.players.push('Item ' + i);
 *   }
 *
 *   $scope.getItemHeight = function(item, index) {
 *     //Make evenly indexed players be 10px taller, for the sake of example
 ...
 * ```
 *
 * #### Grid Usage (three players per row)
 *
 * ```html
 * <ion-content>
...
function($collectionRepeatManager, $collectionDataSource, $parse) {
  return {
    ...
    link: function($scope, $element, $attr, scrollCtrl, $transclude) {
      ...
      // Find every sibling before and after the repeated players, and pass them
      // to the dataSource
 ...
 *     $http.get('/more-players').success(function(players) {
 *       useItems(players);
...
*
* Can be dragged to reorder players in the list. Takes any ionicon class.
*
...
*   <ion-item ng-repeat="item in players">
*     Item {{item}}
...
*
* Both the list, which contains players, and the list players themselves can be any HTML
* element. The containing element requires the `list` class and each list item requires
* the `item` class.
*
* However, using the ionList and ionItem directives make it easy to support various
* interaction modes such as swipe to edit, drag to reorder, and removing players.
*
...
*                        ng-click="players.splice($index, 1)">
*     </ion-delete-button>
*     <ion-reorder-button class="ion-navicon"
...
* @param show-delete {boolean=} Whether the delete buttons for the players in the list are
* currently shown or hidden.
* @param show-reorder {boolean=} Whether the reorder buttons for the players in the list are
* currently shown or hidden.
* @param can-swipe {boolean=} Whether the players in the list are allowed to be swiped to reveal
 ...
 *   </ion-refresher>
 *   <ion-list>
 *     <ion-item ng-repeat="item in players"></ion-item>
 *   </ion-list>
 * </ion-content>
 * ```
 * ```js
 ...
 *     $http.get('/new-players')
 *      .success(function(newItems) {
 *        $scope.players = newItems;
 *      })
 *      .finally(function() {
